...
function init() {
    ...
    git clone git@github.com:seafileltd/ccnet-pro-server.git && cd ccnet-pro-server && ./autogen.sh && ./configure --prefix=$SOURCE_PATH && make && make install && ldconfig && cd ..

    local_migrate

    ccnet-init -c $CONF_PATH -n zming -H 127.0.0.1
    
    git clone git@github.com:seafileltd/seafile-pro-server.git && cd seafile-pro-server && ./autogen.sh && ./configure --disable-fuse --prefix=$SOURCE_PATH && make && make install && ldconfig && cd ..
